...
from tllib.alignment.jan import (
...
def train(args, model, ad_net, train_source_loader, train_target_loader,
          optimizer, optimizer_ad, epoch, method):
    model.train()
    ...
    for batch_idx in range(num_iter):
        ...
        source_tuple = iter_source.next()
        images, labels = source_tuple[0], source_tuple[1]
        data_source, label_source = images.cuda(), labels.cuda()
        ...
        optimizer.zero_grad()
        if method != "JAN" and method != "DAN" and method != "NoTransfer":
            optimizer_ad.zero_grad()

        output, feature = model(data_source)

        loss = nn.CrossEntropyLoss()(output, label_source)
        loss.backward()

        optimizer.step()
        optimizer.zero_grad()

        if method == "NoTransfer":
            continue
        ...
        if method == "CDAN-E":
            ...
            loss2 = loss_func.CDAN([feature, softmax_output], ad_net, None, None, None)
        elif method == "DANN":
            ...
            loss2 = args.trade_off * jmmd_loss(
                (
                    feature[: len(data_source)],
                    F.softmax(output[: len(data_source)], dim=1),
                ),
                (
                    feature[len(data_source) :],
                    F.softmax(output[len(data_source) :], dim=1),
            ...
            loss2 = args.trade_off * mkmmd_loss(
...
def main():
    ...
    # additional parameters for TLlib
    ...
    backbone = tl_utils.get_model(args.mo, pretrain=True)
